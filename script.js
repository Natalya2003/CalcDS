...
function calculateCosts() {
    ...
    if (declaredValue > 0) {
        const declared = +(declaredValue * 0.0001).toFixed(2);
...
function exportToExcel() {
    ...
    const table = document.querySelector('#results-table tbody').rows;
    for (let tr of table) {
        const row = [];
        for (let td of tr.cells) row.push(td.textContent);
        rows.push(row);
    }
    rows.push(['', '', 'Общая сумма:', document.getElementById('total-amount').textContent + ' ' + currentCurrency]);
